...
2. Select the parameters of the elliptic curve:
   ...
   y2 = x3 + ax + b modp
3. Define a base point (generator) G(x, y) on the curve.
4. Choose private keys:
   • User A chooses a random private key a.
   • User B chooses a random private key b.
5. Compute public keys:
   • User A computes public key A = a _ G (scalar multiplication).
   • User B computes public key B = b _ G.
6. Exchange public keys.
7. Each user computes the shared secret:
   • User A computes: S = a _ B
   • User B computes: S = b _ A
   • Since ECC is based on scalar multiplication, both S values are equal.
8. The shared secret is now established.
9. End the program.
